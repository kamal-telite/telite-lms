"""
Check enrollment_requests table structure
"""
import os
import sys
...
def check_enrollment_requests_structure():
    ...
    try:
        ...
        with engine.connect() as conn:
            ...
            print(f"\nEnrollment requests table structure:")
            for column in columns:
                print(f"  {column[0]}: {column[1]}")
                
            # Get some sample data
            print(f"\nSample data:")
            result = conn.execute(text("SELECT * FROM enrollment_requests LIMIT 3"))
...
if __name__ == "__main__":
    check_enrollment_requests_structure()
